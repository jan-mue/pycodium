from pycodium.utils import detect_encoding


def test_get_encoding_utf8() -> None:
    text = b"# coding: utf-8\nprint('hi')"
    assert detect_encoding.get_encoding(text) == "utf-8"


def test_get_encoding_default() -> None:
    text = b"print('hi')"
    assert detect_encoding.get_encoding(text, default_encoding="utf-8") == "utf-8"


def test_decode_utf8() -> None:
    text = b"hello"
    decoded, encoding = detect_encoding.decode(text)
